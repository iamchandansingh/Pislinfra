...
const getIcon = (name) => {
  const size = 26;
  switch (name) {
    case 'FiBookOpen': return <FiBookOpen size={size} />;
    case 'FiHeart': return <FiHeart size={size} />;
    case 'FiFeather': return <FiFeather size={size} />;
    case 'FiUsers': return <FiUsers size={size} />;
    default: return <FiUsers size={size} />;
  }
};
...
const CSRFocusAreas = ({ areas }) => {
  const focusAreas = areas && areas.length > 0 ? areas : [];
              ...
              <div style={{ padding: '20px 16px', flex: 1 }}>
                <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '14px' }}>
                  <div style={{ width: '56px', height: '56px', borderRadius: '50%', backgroundColor: '#F8FAFC', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <span style={{ color: area.iconColor, display: 'flex', lineHeight: 0 }}>{getIcon(area.iconName)}</span>
                  </div>
                </div>
