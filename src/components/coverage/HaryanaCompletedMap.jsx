...
const handleCardClick=(project)=>{const slug=project.name.toLowerCase().replace(/\s+/g,'-').replace(/[^a-z0-9-]/g,'');window.location.href=`/project/completed/${slug}`;};
...
<div className="hcm-header"><div className="hcm-header-top"><div><h1 className="hcm-main-title">Haryana <span style={{color:GREEN}}>Completed</span> Projects</h1><p className="hcm-subtitle">Successfully delivered industrial & logistics parks across Haryana.</p></div><div style={{display:'flex',alignItems:'center',gap:8,padding:'10px 18px',borderRadius:10,border:'2px solid #198847',background:'#f0fdf4',color:GREEN,fontWeight:700,fontSize:14}}><CheckCircle2 size={15}/>Completed<span style={{background:GREEN,color:'#fff',borderRadius:20,padding:'2px 10px',fontSize:12,fontWeight:800}}>{haryanaCompletedProjects.length}</span></div></div></div>
...
.hcm-card-selected{border:3px solid #198847!important;box-shadow:0 0 0 4px rgba(25,136,71,0.12)!important;}
