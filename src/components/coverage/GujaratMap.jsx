...
const handleCardClick=(project)=>{const slug=project.name.toLowerCase().replace(/\s+/g,'-').replace(/[^a-z0-9-]/g,'');navigate(`/project/ongoing/${slug}`,{state:{projectData:project}});};
...
<div className="gcm-header"><div className="gcm-header-top"><div><h1 className="gcm-main-title">Gujarat <span style={{color:ORANGE}}>Ongoing</span> Projects</h1><p className="gcm-subtitle">Real-time monitoring of infrastructure development across Gujarat.</p></div><div style={{display:'flex',alignItems:'center',gap:8,padding:'8px 16px',borderRadius:10,border:'1.5px solid #f97316',background:'#fff5f0',color:ORANGE,fontWeight:700,fontSize:13}}><Activity size={14}/>Ongoing<span style={{background:ORANGE,color:'#fff',borderRadius:20,padding:'2px 10px',fontSize:12,fontWeight:800}}>{gujaratOngoingProjects.length}</span></div></div></div>
...
.gcm-card-selected{border:1.5px solid #f97316!important;box-shadow:0 6px 20px rgba(249,115,22,0.15)!important;}
