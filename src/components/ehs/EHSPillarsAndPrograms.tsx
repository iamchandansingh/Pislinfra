...
import { FiShield, FiFeather, FiHeart, FiBell } from 'react-icons/fi';
...
const EHSPillarsAndPrograms = ({ pillars, title }) => {
  const activePillars = pillars && pillars.length > 0 ? pillars.map((p, i) => ({ id: p.id || i, icon: pillarsData[i]?.icon, iconColor: pillarsData[i]?.iconColor, title: p.title, description: p.description })) : pillarsData;
  return (
    <div style={{ width: '100%', padding: '20px 0 30px', backgroundColor: '#FFFFFF' }}>
      <div style={{ width: '98%', maxWidth: '1370px', margin: '0 auto' }}>
        
        <h2 style={{ fontSize: '36px', fontWeight: 800, color: '#052A73', letterSpacing: '-1px', lineHeight: 1.1, margin: 0, fontFamily: 'Inter, sans-serif' }}>{title || "Our EHS Pillars"}</h2>
